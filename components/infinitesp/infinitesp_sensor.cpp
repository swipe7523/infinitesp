...
void InfinitESPSensor::on_register_update(uint8_t device_addr, uint16_t register_key) {
  ...
  if (register_key == REG_SAM_STATE) {
    ...
    // 0xFF is the "no sensor / not ready" sentinel — the thermostat reports it for
    // unequipped zones and, transiently, for ALL sensors during its post-reboot
    // warmup. Decoding it raw yields garbage (255 °F → 123.9 °C), so leave value
    // NAN: the publish below is gated on !isnan, holding the last good reading.
  ...
  if (register_key == REG_ODU_FAN) {
    struct FanField {
      ...
      bool temp_f;  // native °F → °C
    };
    static const FanField fan_fields[] = {
        {"odu_fan_rpm", InfinitESPComponent::odu_outdoor_fan_rpm_, false},
        {"odu_dc_bus_voltage", InfinitESPComponent::odu_dc_bus_voltage_, false},
        {"odu_ac_line_current", InfinitESPComponent::odu_ac_line_current_, false},
        {"odu_ipm_temp", InfinitESPComponent::odu_ipm_temp_f_, true},
        {"odu_pfcm_temp", InfinitESPComponent::odu_pfcm_temp_f_, true},
    };
    for (const auto &fld : fan_fields) {
      if (sensor_type_ != fld.type)
        continue;
      auto *data = parent_->get_register(device_addr, REG_ODU_FAN);
      if (data) {
        float v = fld.decode(*data);
        if (!std::isnan(v))
          value = fld.temp_f ? f_to_c(v) : v;
      }
      break;
    }
  }
  ...
  if (register_key == REG_ODU_STATUS2) {
    ...
    static const PressField press_fields[] = {
        {"odu_suction_pressure", InfinitESPComponent::odu_suction_pressure_psig_},
        {"odu_discharge_pressure", InfinitESPComponent::odu_discharge_pressure_psig_},
    };
    for (const auto &fld : press_fields) {
      if (sensor_type_ != fld.type)
        continue;
      auto *data = parent_->get_register(device_addr, REG_ODU_STATUS2);
      if (data)
        value = fld.decode(*data);
      break;
    }
  }
  ...
  if (register_key == REG_ODU_POWER && sensor_type_ == "odu_power") {
    ...
    if (data)
      value = InfinitESPComponent::odu_power_w_(*data);
  }
